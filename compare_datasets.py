...
Usage:
    python compare_datasets.py                    # Full comparison
    python compare_datasets.py --output chart.png  # Custom output path
    python compare_datasets.py --summary           # Text summary only
"""
...
def print_summary(runs):
    ...
    print("\n" + "=" * 80)
    print("  CROSS-DATASET COMPARISON (Gaudi 3)")
    print("=" * 80)
...
def generate_chart(runs, output_path):
    ...
    fig.suptitle("Multi-Dataset Experiment Comparison (Gaudi 3)", fontsize=16, fontweight="bold", y=0.98)
    ...
    ax2.bar([i - 0.15 for i in x], baselines, 0.3, label="Baseline", color="lightgray", edgecolor="gray")
    ax2.bar([i + 0.15 for i in x], bests, 0.3, label="Best",
...
def main():
    parser = argparse.ArgumentParser(description="Cross-dataset comparison tool (Gaudi 3)")
