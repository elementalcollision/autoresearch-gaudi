...
Usage:
    ...
    python convert_dataset.py fineweb-edu

    # More shards for longer runs
    python convert_dataset.py fineweb-edu --num-shards 50

    # Dry run (show what would be downloaded)
    python convert_dataset.py fineweb-edu --dry-run

    # Restore original climbmix dataset
    python convert_dataset.py --restore
...
After conversion, run: python prepare.py
...
DATASETS = {
    ...
    "fineweb": {
        "description": "FineWeb: unfiltered web crawl, 10BT sample (no quality scoring)",
...
def rechunk_to_shards(source_dir, dataset_config, num_shards, output_dir):
    """Read source parquet files, extract text column, and write as shards."""
...
Examples:
  python convert_dataset.py fineweb-edu
  python convert_dataset.py fineweb
  python convert_dataset.py github-code-python --num-shards 10
  python convert_dataset.py cosmopedia-v2 --num-shards 10 --num-source 3
  python convert_dataset.py --restore
        """,
    )
    ...
    parser.add_argument("--num-source", type=int, default=3,
                        help="Number of source files to download (default: 3)")
    ...
    parser.add_argument("--skip-backup", action="store_true",
                        help="Skip backing up current dataset")
    ...
    print(f"")
    print(f"Next steps:")
    print(f"  1. Train tokenizer:  python prepare.py")
    print(f"  2. Run experiment:   python train_gaudi.py")
    print(f"  3. Or agent mode:    python dashboard.py --agent --tag fineweb-edu")
    print(f"")
    print(f"To restore the original dataset:")
    print(f"  python convert_dataset.py --restore")
    print(f"{'='*60}")
