...
This version targets Intel Gaudi 3 HPU (PyTorch only, spawn-safe multiprocessing).
"""
...
def download_data(num_shards, download_workers=8):
    ...
    # Use spawn context for Gaudi compatibility (required by habanalabs runtime)
...
# ---------------------------------------------------------------------------
# Runtime utilities (imported by train_gaudi.py)
# ---------------------------------------------------------------------------
...
def get_token_bytes(backend=None):
    """Load token_bytes lookup. Returns tensor on HPU device."""
    ...
    import torch
    device = "hpu" if backend == "hpu" else "cpu"
...
def make_dataloader(tokenizer, B, T, split, buffer_size=1000, backend=None):
    ...
    backend: 'hpu' or None (defaults to hpu)
    """
    if backend is None:
        backend = "hpu"
