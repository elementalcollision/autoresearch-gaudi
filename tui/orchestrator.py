...
# Marker comments that delimit the hyperparameter block in train_gaudi.py
HP_BLOCK_START = "# ---------------------------------------------------------------------------\n# Hyperparameters"
HP_BLOCK_END = "# ---------------------------------------------------------------------------\n# Setup"
...
class OrchestratorCallbacks:
    ...
    on_experiment_start: Callable[[int, str, str], None]
    on_training_output: Callable[[str], None]
    on_experiment_complete: Callable[['ExperimentResult'], None]
    on_stats_update: Callable[[int, int, int, float], None]
...
class ExperimentOrchestrator:
    ...
    def _cb_status(self, status, message):
        if self._callbacks:
            self._callbacks.on_status_change(status, message)

    def _cb_experiment_start(self, exp_num, desc, reasoning):
        if self._callbacks:
            self._callbacks.on_experiment_start(exp_num, desc, reasoning)

    def _cb_training_output(self, line):
        if self._callbacks:
            self._callbacks.on_training_output(line)

    def _cb_experiment_complete(self, result):
        if self._callbacks:
            self._callbacks.on_experiment_complete(result)

    def _cb_stats_update(self):
        if self._callbacks:
            self._callbacks.on_stats_update(
                self.total_runs, self.kept_count, self.discarded_count, self.best_val_bpb)
        ...
        try:
            ...
            try:
                if not self._llm.validate():
                    self._cb_error("API credential validation failed.")
                    return
            except Exception as e:
            ...
            self._cb_error(f"Traceback: {traceback.format_exc()}")
        finally:
            self._running = False
    ...
    def _run_baseline(self) -> None:
        ...
        if final:
            result = ExperimentResult(
                ...
                val_bpb=final.val_bpb, peak_mem_gb=final.peak_vram_mb / 1024,
    ...
    def _run_experiment(self, exp_num: int) -> None:
        ...
        for attempt in range(3):
            try:
                proposal = self._llm.generate_experiment(
                    current_code=current_code, results_history=results_history,
                    best_val_bpb=self.best_val_bpb, best_experiment=self.best_experiment,
        ...
        try:
            commit_hash = self._git.commit_changes(
                f"exp{exp_num}: {proposal.description}", [self._training_script])
        except Exception as e:
